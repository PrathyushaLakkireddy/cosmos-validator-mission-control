...
func GaiadVersion(_ HTTPOptions, cfg *config.Config, c client.Client) {
	...
	cmd := exec.Command(cfg.DaemonName, "version", "--long")
	out, err := cmd.CombinedOutput()
	if err != nil {
		log.Printf("cmd.Run() failed with %s\n", err)
		_ = writeToInfluxDb(c, bp, "vcf_daemon_version", map[string]string{}, map[string]interface{}{"v": "NA"})
		return
	}
	...
	_ = writeToInfluxDb(c, bp, "vcf_daemon_version", map[string]string{}, map[string]interface{}{"v": matches[0][1]})
